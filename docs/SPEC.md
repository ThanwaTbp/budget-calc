...
- แท็บกรองสถานะ (ทั้งหมด/ค้างอยู่/เสร็จแล้ว) ใช้กับมุมมองรายวัน/รายเดือน และแผงรายละเอียดวันที่เลือกใต้ปฏิทิน
- หัวข้อของแผงเปลี่ยนตามมุมมอง (รายวัน = ชื่อวัน · รายเดือน/ปฏิทิน = ชื่อเดือน พ.ศ.)
...
# Redesign หน้าตรวจหวย — v7

## แนวคิด: ใช้ภาษาของ "ใบสลาก" และ "กระดานประกาศผลรางวัล"
หน้านี้มีอารมณ์ชัดเจนกว่าหน้าอื่นในแอป — ผู้ใช้เข้ามาเพื่อถามคำถามเดียว **"ถูกรางวัลไหม"**
ดังนั้นลำดับความสำคัญคือ **ช่องตรวจเลข > คำตอบ > ผลรางวัลทั้งงวด > เลขที่บันทึกไว้**

## โครงหน้าใหม่ (เปลี่ยนจากเดิมที่ยัดฟอร์มไว้ในคอลัมน์ซ้ายแคบๆ)
```
PageHeader: ตรวจหวย                                   [เลือกงวด ▾]
─────────────────────────────────────────────────────────────────
HERO เต็มความกว้าง — การ์ดตรวจเลข
   งวดวันที่ 16 สิงหาคม 2569
   [0][0][4][6][1][5]        [ ตรวจรางวัล ]
   ┌ ผลตรวจ ────────────────────────────────┐
   │ ถูกรางวัลที่ 1        6,000,000 บาท     │
   │ รวมทั้งสิ้น           6,000,000 บาท     │
   └────────────────────────────────────────┘
─────────────────────────────────────────────────────────────────
[ ผลรางวัลงวดนี้  (2 ส่วน 3) ]   [ เลขที่บันทึกไว้ (1 ส่วน 3) ]
```

## รายละเอียดที่ต้องทำให้ได้
1. **ช่องกรอกเลขเป็นกล่องแยก 6 หลัก** ใช้ `InputOTP` (`@/components/ui/input-otp` — ติดตั้งแล้ว) `maxLength={6}` `pattern` ตัวเลขเท่านั้น กล่องใหญ่ `size-14` ตัวเลข `text-2xl tabular font-semibold`
2. **รางวัลที่ 1 เป็นพระเอกของแผงผลรางวัล** — เลขตัวใหญ่มาก `text-4xl sm:text-5xl tabular tracking-[0.2em] font-bold` อยู่ในกรอบ `border-2 border-dashed` (สื่อถึงขอบใบสลาก)
3. **เลขหน้า 3 ตัว / เลขท้าย 3 ตัว / เลขท้าย 2 ตัว** อยู่ใน grid 3 ช่องเสมอกัน — คนตรวจบ่อยที่สุด ต้องเห็นโดยไม่ต้องกาง
4. **ข้างเคียงรางวัลที่ 1 และรางวัลที่ 2-5** ยุบเป็นแถวกดกางได้ บอกจำนวนเลขในวงเล็บ
5. **สถานะถูก/ไม่ถูก**: ถูก → พื้น `bg-income-muted` ขอบ `border-income/40` ยอดเงินตัวใหญ่ `text-income` · ไม่ถูก → พื้น `bg-muted/40` ข้อความสุภาพ ไม่ดราม่า
6. **ไฮไลต์เลขที่บันทึกไว้แล้วถูกรางวัล** ในแผงผลรางวัลด้วย `bg-income-muted text-income ring-1 ring-income/40`
